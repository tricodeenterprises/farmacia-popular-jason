...
export default function SetupCiclo({ paciente, cicloId, onComplete, onCancel, galeriaFotos = [], galeriaLoading = false }: Props) {
      ...
      const { data, error } = await supabase.functions.invoke("ocr-document", {
        body: {
          ...
          contexto_ciclo: {
            ...
            intervalo_dias: tipoReceita === "fralda" ? 10 : 29,
  ...
  const canGoNext = (): boolean => {
    switch (currentStep) {
      case "receita":
        return !!receitaFile && !!dataEmissao && !!nomeMedico && !getReceitaVencida();
      ...
      case "cupom_fiscal": return !!cupomFile && !!cupomQrFile && !!cupomDataCompra && cupomDatasConfirmadas && dispensadoHoje !== null && !(dataEmissao && cupomDataCompra < dataEmissao);
  ...
  const runOcrReceita = async (base64: string) => {
    ...
    try {
      ...
      if (error) throw error;
      if (data.erro && !data.data_emissao && !data.crm && !data.nome_medico) { setOcrReceitaStatus("error"); setOcrReceitaDetails(["❌ " + data.erro, ...getQualityFeedback(data)]); return; }
      ...
      if (data.nome_medico) { setNomeMedico(data.nome_medico); details.push("✅ Médico: " + data.nome_medico); } else details.push("❌ Médico não encontrado");
      ...
      const hasData = data.data_emissao || data.crm || data.nome_medico;
  ...
  const handleSave = async () => {
    ...
    if (needsReceita && (!receitaFile || !dataEmissao || !nomeMedico)) { toast.error("Dados da receita incompletos."); return; }
    ...
    try {
      ...
      if (needsReceita) {
        ...
        if (sameTypeCycles.length >= 2) {
          toast.error(`Limite de 2 ciclos ativos de ${tipoReceita === "fralda" ? "Fralda" : "Medicamento"} atingido. Encerre um ciclo antes de registrar outra receita.`);
      ...
      if (needsReceita && receitaFile) {
        ...
        // Register doctor removed - no CRM needed
        ...
        const { data: receita, error: recErr } = await supabase.from("receitas").insert({
          ...
          validade_ate: validadeStr, crm: null, nome_medico: nomeMedico || null,
        ...
        {!preview ? (
          ...
          <div className="space-y-2">
            <div className="relative rounded-xl overflow-hidden border bg-muted/20">
              <img src={preview} alt={label} className="w-full max-h-36 object-contain p-2" />
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl border-2 border-dashed border-primary/40 bg-primary/5 flex flex-col max-h-[80vh] overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }} className="min-h-[200px]">
            ...
            {currentStep === "receita" && (
              <div className="space-y-4">
                ...
                {/* Médico */}
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Médico Prescritor *</Label>
                  {nomeMedico ? (
                    <div className="flex items-center gap-2 p-2.5 rounded-xl border bg-muted/50">
                      <div className="flex-1">
                        <p className="font-medium text-sm">{nomeMedico}</p>
                      </div>
                      <Button variant="ghost" size="sm" className="rounded-lg text-xs"
                        onClick={() => setNomeMedico("")}>
                        Alterar
                      </Button>
                    </div>
                  ) : (
                    <Input value={nomeMedico} onChange={(e) => setNomeMedico(e.target.value)}
                      placeholder="Nome do médico prescritor..." className="rounded-xl" autoComplete="off" />
                  )}
                </div>
              </div>
            )}
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      {!cupomQrPreview ? (
                        ...
                        <div className="space-y-2">
                          <div className="relative rounded-xl overflow-hidden border bg-muted/20">
                            <img src={cupomQrPreview} alt="Cupom QR Code" className="w-full max-h-36 object-contain p-2" />
                            <div className="absolute top-2 right-2">
                              <div className="bg-background/90 rounded-full p-1.5 shadow-sm">
                                <CheckCircle className="w-5 h-5 text-green-500" />
                              </div>
                ...
                <div className="space-y-1.5 p-3 rounded-2xl bg-muted/40 text-sm">
                  {[
                    ...
                    ...(needsReceita ? [
                      ...
                      ["Tipo", tipoReceita === "fralda" ? "🧷 Fralda" : "💊 Medicamento"],
                      ["Médico", nomeMedico],
                    ] : []),
        ...
        {/* Sticky Navigation */}
        <div className="flex items-center justify-between p-4 border-t border-primary/20 bg-background/80 backdrop-blur-sm shrink-0">
          {currentStepIndex > 0 ? (
