...
import { addDaysToDateStr, buildCycleRecalculationFromDispensacoes, calculateCycleLimit, calculateNextWithdrawalDate, getCycleClosureInfo, compareDateStr, getTodayLocalDateStr, getCycleValidityDate } from "@/lib/ciclo-utils";
import { CATEGORIAS, CATEGORY_CONFIG, capMaximoFor, categoriaLabelComEmoji, getCategoria, normalizeCategoria, type CategoriaDispensacao } from "@/lib/categorias";
import { checkPastaReorganizacao, fetchPastaNumeroPaciente, normalizeInitial, type PastaNumero } from "@/lib/pasta-numeracao";


function validateCicloParaDispensacao(ciclo: any): { ok: boolean; motivo?: string } {
  if (!ciclo) return { ok: false, motivo: "Ciclo não encontrado." };
  const validade = getCycleValidityDate(ciclo);
  const hoje = getTodayLocalDateStr();
  if (validade && compareDateStr(hoje, validade) > 0) {
    return { ok: false, motivo: `Receita vencida em ${validade.split("-").reverse().join("/")}. Encerre o ciclo e registre uma nova receita.` };
  }
  const total = Number(ciclo.total_dispensacoes || 0);
  const limite = Number(ciclo.limite_maximo || 0);
  if (limite > 0 && total >= limite) {
    return { ok: false, motivo: `Limite máximo de ${limite} retiradas atingido neste ciclo. Encerre o ciclo para registrar uma nova receita.` };
  }
  return { ok: true };
}
...
export default function PacientePanel({ paciente: pacienteProp }: Props) {
  const { isMaster, user, effectiveRole } = useAuth();
  ...
  // Type chooser (4 categorias) — sempre aparece ao abrir o perfil
  const [showTypeChooser, setShowTypeChooser] = useState(true);
  const [typeChooserResult, setTypeChooserResult] = useState<null | { tipo: CategoriaDispensacao; found: boolean }>(null);
  const [pastaInfo, setPastaInfo] = useState<PastaNumero | null>(null);
  ...
  const fetchData = async () => {
    ...
    // Não encerra ciclo automaticamente ao abrir a tela.
    // O encerramento agora é manual e deve ser acionado quando a próxima retirada ultrapassar a validade.
    ...
    if (isMaster) {
      allCiclosData = await Promise.all(allCiclosData.map(async (c: any) => {
        ...
        const { error } = await supabase.from("ciclos").update(updates as any).eq("id", c.id);
        if (error) {
  ...
  useEffect(() => {
    let alive = true;
    fetchPastaNumeroPaciente(paciente.id, paciente.nome)
      .then((info) => { if (alive) setPastaInfo(info); })
      .catch(() => { if (alive) setPastaInfo(null); });
    return () => { alive = false; };
  ...
  const calcStatusForCiclo = (ciclo: any) => {
    ...
    const fechamento = getCycleClosureInfo(ciclo);
    const restantes = Math.max(0, ciclo.limite_maximo - ciclo.total_dispensacoes);
    const proximaStr = ciclo.ultima_retirada
      ? calculateNextWithdrawalDate(ciclo.ultima_retirada, ciclo.intervalo_dias)
      : ciclo.data_inicio;
    if (fechamento.shouldClose) {
      return {
        label: "ENCERRAMENTO NECESSÁRIO",
        color: "status-bloqueado",
        canDispense: false,
        actionLabel: "Encerrar ciclo",
        info: fechamento.proximaUltrapassaValidade
          ? `A próxima retirada (${formatDateBR(fechamento.proximaRetirada || "")}) ultrapassa a validade (${formatDateBR(fechamento.validade || ciclo.data_fim)}). Encerre o ciclo manualmente.`
          : `Máximo de ${ciclo.limite_maximo} retiradas atingido. Encerre o ciclo manualmente.`,
        proximaRetirada: fechamento.proximaRetirada,
        reasons: [fechamento.label],
      };
    }
    if (proximaStr) {
  ...
  const primaryActionLabel = !cicloAtivo
    ...
    : canDispenseAll
      ...
      : status.label === "ENCERRAMENTO NECESSÁRIO"
        ? "Encerrar ciclo"
  ...
  const runPrimaryAction = () => {
    ...
    if (status.label === "ENCERRAMENTO NECESSÁRIO") {
      setMotivoEncerramento(getCycleClosureInfo(cicloAtivo).label);
  ...
  const handleEncerrarCiclo = async () => {
    ...
    setEncerrando(true);
    const fechamento = getCycleClosureInfo(cicloAtivo);
    const motivoSistema = fechamento.motivo || "manual";
    const { error } = await supabase.from("ciclos").update({
      status: "encerrado",
      motivo_encerramento: motivoSistema,
      encerrado_em: new Date().toISOString(),
      encerrado_por: user?.id,
    }).eq("id", cicloAtivo.id);
    if (error) { toast.error("Erro ao encerrar ciclo."); setEncerrando(false); return; }
    try {
      const reorg = await checkPastaReorganizacao(paciente.nome);
      if (reorg) toast.info(`Pasta "${reorg.letra}" precisa de reimpressão (${reorg.pessoas} pessoas renumeradas).`);
    } catch { /* noop */ }
    ...
    await supabase.from("logs").insert([{
      ...
      detalhes: {
        ciclo_id: cicloAtivo.id,
        paciente_id: paciente.id,
        motivo_digitado: motivoEncerramento.trim(),
        motivo_sistema: motivoSistema,
        proxima_retirada: fechamento.proximaRetirada,
        validade: fechamento.validade,
      } as any,
    }]);
  ...
  const recalcularCicloAposAlteracaoDispensacao = async (cicloId: string) => {
    const [{ data: cicloAtual, error: cicloFetchError }, { data: dispensacoesCiclo, error: dispFetchError }] = await Promise.all([
      supabase.from("ciclos").select("*, receitas(data_emissao)").eq("id", cicloId).single(),
      supabase
        .from("dispensacoes")
        .select("id, created_at, data_dispensacao_real, cancelada")
        .eq("ciclo_id", cicloId)
        .order("data_dispensacao_real", { ascending: true, nullsFirst: false })
        .order("created_at", { ascending: true }),
    ]);

    if (cicloFetchError || dispFetchError) {
      throw new Error(cicloFetchError?.message || dispFetchError?.message || "Erro ao buscar dados do ciclo.");
    }

    const recalculo = buildCycleRecalculationFromDispensacoes(dispensacoesCiclo || [], cicloAtual);
    const { error: updateError } = await supabase.from("ciclos").update(recalculo.updates as any).eq("id", cicloId);
  ...
  const handleCancelarDispensacao = async () => {
    ...
    let recalculo: Awaited<ReturnType<typeof recalcularCicloAposAlteracaoDispensacao>> | null = null;
    if (disp?.ciclo_id) {
      try {
        recalculo = await recalcularCicloAposAlteracaoDispensacao(disp.ciclo_id);
      } catch (err: any) {
        toast.error("Dispensação cancelada, mas houve erro ao recalcular o ciclo. Atualize antes de registrar nova retirada.");
        setCancelando(false);
        return;
      }
    }
    await supabase.from("logs").insert([{
      ...
      detalhes: {
        dispensacao_id: cancelDispId,
        justificativa: justificativaCancelamento.trim(),
        paciente_id: paciente.id,
        ciclo_id: disp?.ciclo_id || null,
        recalculo_ciclo: recalculo ? {
          total_dispensacoes: recalculo.total,
          ultima_retirada: recalculo.ultimaRetirada,
          primeira_retirada: recalculo.primeiraRetirada,
        } : null,
      } as any,
    }]);
    toast.success("Dispensação cancelada e ciclo recalculado.");
  ...
  const doPrintCupom = (d: any, posicao: number, itens: any[]) => {
    ...
    const tipoReceita = normalizeCategoria((ciclo?.receitas as any)?.tipo);
    const limiteInicial = capMaximoFor(tipoReceita);
    ...
    const ok = printDispensacaoCupom({
      ...
      pastaLetra: pastaInfo?.letra || normalizeInitial(paciente.nome),
      pastaNumero: pastaInfo?.numero ?? null,
      itens,
      numero: posicao,
      totalCiclo,
  ...
  const handleNovoAtendimento = () => {
    if (activeCiclos.length <= 1) {
      const check = validateCicloParaDispensacao(cicloAtivo);
  ...
  const confirmCycleSelection = (cycle?: any) => {
    ...
    if (selected) {
      const check = validateCicloParaDispensacao(selected);
      if (!check.ok) { toast.error(check.motivo!); return; }
  ...
  const countByCategoria = (cat: CategoriaDispensacao) =>
    activeCiclos.filter((c: any) => normalizeCategoria((c.receitas as any)?.tipo) === cat).length;

  // Check if can add new cycle (max 2 per category)
  const canAddNewCycle = () => CATEGORIAS.some((cat) => countByCategoria(cat) < 2);

  const getMaxCycleTypeMessage = () => {
    const msgs = CATEGORIAS.filter((cat) => countByCategoria(cat) >= 2).map((cat) => `${CATEGORY_CONFIG[cat].label} (2/2)`);
    return msgs.length > 0 ? `Limite atingido: ${msgs.join(", ")}` : null;
  };

  const handleChooseType = (tipo: CategoriaDispensacao) => {
    const match = activeCiclos.find((c: any) => normalizeCategoria((c.receitas as any)?.tipo) === tipo);
    if (match) {
      setCicloAtivo(match);
      setTypeChooserResult({ tipo, found: true });
      setShowTypeChooser(false);
    } else {
      setTypeChooserResult({ tipo, found: false });
    }
  };
  ...
  return (
    ...
    />
    {/* Type Chooser Dialog — Medicamento / Fralda */}
    {!loading && showTypeChooser && (
      <Dialog open onOpenChange={(open) => { if (!open) { setShowTypeChooser(false); setTypeChooserResult(null); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-primary">
              <Package className="w-5 h-5" /> O que deseja atender?
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Escolha o tipo de retirada para <strong>{paciente.nome}</strong>.
            </p>
            <div className="grid grid-cols-2 gap-3">
              {CATEGORIAS.map((cat) => {
                const cfg = CATEGORY_CONFIG[cat];
                const temAtivo = activeCiclos.some((c: any) => normalizeCategoria((c.receitas as any)?.tipo) === cat);
                return (
                  <Button
                    key={cat}
                    onClick={() => handleChooseType(cat)}
                    variant="outline"
                    className="h-24 rounded-2xl text-base font-black flex flex-col items-center justify-center gap-1 border-4 text-white"
                    style={{ borderColor: cfg.color, background: cfg.color }}
                  >
                    <span className="text-2xl">{cfg.emoji}</span>
                    {cfg.label}
                    <span className="text-[10px] font-bold opacity-90">{temAtivo ? "ciclo ativo" : "sem ciclo ativo"}</span>
                  </Button>
                );
              })}
            </div>
            {typeChooserResult && !typeChooserResult.found && (
              <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
                <p className="font-semibold flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4" />
                  Nenhum ciclo ativo de {CATEGORY_CONFIG[typeChooserResult.tipo].label.toLowerCase()}.
                </p>

                <p className="text-xs mt-1">
                  O cliente não possui ciclo ativo desse tipo, ou o ciclo já foi encerrado. Você pode cadastrar uma nova receita ou consultar o histórico.
                </p>
                <div className="flex gap-2 mt-3">
                  <Button
                    size="sm"
                    onClick={() => { setShowTypeChooser(false); setTypeChooserResult(null); setShowSetupNovo(true); }}
                    className="flex-1 rounded-xl"
                    disabled={!canAddNewCycle()}
                  >
                    Cadastrar receita
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => { setShowTypeChooser(false); setTypeChooserResult(null); }}
                    className="flex-1 rounded-xl"
                  >
                    Ver perfil
                  </Button>
            ...
            <button
              onClick={() => { setShowTypeChooser(false); setTypeChooserResult(null); }}
              className="w-full text-xs text-muted-foreground underline pt-2"
            >
              Pular seleção e abrir perfil completo
            </button>
          </div>
        </DialogContent>
      </Dialog>
    ...
    <div className="w-full max-w-[1480px] mx-auto space-y-6">
      ...
      {activeCiclos.length > 1 ? (
        /* Cards por categoria (Medicamento / Fralda / Alendronato / Absorvente) */
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          {activeCiclos.map((c: any) => {
            const st = calcStatusForCiclo(c);
            const cat = getCategoria((c.receitas as any)?.tipo);
            ...
            return (
              <motion.button
                ...
                className={`relative rounded-2xl overflow-hidden text-left transition-all border-4 ${isSelected ? "shadow-xl scale-[1.01]" : "opacity-75 hover:opacity-100"}`}
                style={{ borderColor: cat.color }}
              >
                <div className="px-3 py-1.5 text-center text-[11px] font-black uppercase tracking-widest text-white" style={{ background: cat.color }}>
                  {cat.emoji} {cat.labelPlural}
                </div>
                <div className={`${st.color === "status-bloqueado"
                  ...
                  : st.color} p-3 sm:p-4 text-center`}>
                  ...
                  {st.proximaRetirada && <p className="text-sm font-semibold mt-1">📅 Próxima: {formatDateBR(st.proximaRetirada)}</p>}
                  <p className="text-[11px] mt-1 opacity-80">Validade: {formatDateBR(c.data_fim)} · Retiradas: {c.total_dispensacoes}/{c.limite_maximo}</p>
                </div>
              </motion.button>
            );
          })}
            ...
            <div className="relative bg-gradient-to-br from-[hsl(260,60%,8%)] via-[hsl(270,50%,12%)] to-[hsl(280,40%,6%)] rounded-2xl p-4 sm:p-6 text-center border border-violet-500/20 shadow-[0_0_40px_-10px_hsl(270,80%,60%,0.3)]">
              ...
              <div className="relative z-10">
                {cicloAtivo && <p className="text-xs font-bold uppercase tracking-widest mb-2 text-violet-300/60">
                  {categoriaLabelComEmoji((cicloAtivo.receitas as any)?.tipo)}
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="hero-card rounded-[1.6rem] p-4 sm:p-5 space-y-4">
        ...
        {cicloAtivo && getCycleClosureInfo(cicloAtivo).shouldClose && (
          <div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive space-y-1">
            <p className="font-bold">Este ciclo deve ser encerrado manualmente.</p>
            <p>Regra: a próxima retirada ultrapassou a data de vencimento da receita ou o limite de retiradas foi atingido.</p>
          </div>
        )}

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <Button size="lg" onClick={runPrimaryAction} disabled={encerrandoEAbrindo || (!canDispenseAll && status.label.startsWith("AGUARDAR"))} className="min-h-[52px] rounded-2xl text-base font-bold">
          ...
          {paciente.telefone ? (
            ...
            <Button variant="outline" size="lg" className="min-h-[52px] rounded-2xl text-base font-bold" onClick={() => setShowPhoneGate(true)}>
              <Phone className="w-5 h-5 mr-2" /> Cadastrar telefone
            </Button>
          )}
        </div>
      </motion.div>
            ...
            <div className="px-4 pb-4 space-y-2 border-t border-border/30 pt-3">
              {editing ? (
                ...
                <div className="space-y-2">
                  ...
                  {paciente.telefone && (
                    <a href={buildWhatsAppUrl(`Prezado(a) Senhor(a), ${paciente.nome.split(" ")[0]}.`)} target="_blank" rel="noopener noreferrer">
      ...
      {/* ═══ Ciclo Ativo ═══ */}
      <div className={showSetupNovo && !cicloAtivo ? "relative" : "rounded-2xl border border-border/50 bg-card overflow-hidden relative"}>
            ...
            <div className="flex items-center justify-between p-4 pb-3">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-base font-bold" style={{ fontFamily: "var(--font-display)" }}>
                  {activeCiclos.length === 0 ? "Nenhum ciclo ativo" : activeCiclos.length > 1 ? "Ciclos ativos" : "Ciclo ativo"}
                </h3>
              </div>
            </div>
          </>
            ...
            {activeCiclos.map((c: any) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  className={`rounded-xl text-xs flex-1 min-w-[120px] border-2 font-bold ${isSelected ? cat.badgeClass : "opacity-70"}`}
                  style={isSelected ? undefined : { borderColor: cat.color, color: cat.color }}
                  onClick={() => setCicloAtivo(c)}
                >
                  {cat.emoji} {cat.label}
                </Button>
        ...
        <div className={showSetupNovo && !cicloAtivo ? "p-0" : "px-4 pb-4"}>
          ...
          {cicloAtivo ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Tipo", value: categoriaLabelComEmoji((cicloAtivo.receitas as any)?.tipo) },
            ...
            <div className="space-y-4">
              {showSetupNovo ? (
                ...
                >
                  ...
                  <div className="mt-5 rounded-2xl border border-primary/20 bg-white/70 p-3 text-center">
                    <p className="text-xs font-semibold text-muted-foreground">
                      Use a ação principal acima para cadastrar a primeira receita.
                    </p>
                  </div>
      ...
      {/* Setup Novo Ciclo - tela expandida quando já existe ciclo ativo */}
      {showSetupNovo && cicloAtivo && (
        <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
          <div className="min-h-full w-full p-3 sm:p-6 lg:p-8">
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 space-y-1">
                ...
                {cicloAtivo && getCycleClosureInfo(cicloAtivo).proximaRetirada && (
                  <p className="text-xs text-muted-foreground">Próxima retirada calculada: {formatDateBR(getCycleClosureInfo(cicloAtivo).proximaRetirada || "")}</p>
                )}
