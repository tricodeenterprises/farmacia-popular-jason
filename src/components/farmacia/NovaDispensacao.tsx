...
import { normalizeCategoria } from "@/lib/categorias";
import { fetchPastaNumeroPaciente, normalizeInitial } from "@/lib/pasta-numeracao";
import { calculateCycleLimit, calculateNextWithdrawalDate, getTodayLocalDateStr, isFutureDateStr, getCycleValidityDate, compareDateStr, addDaysToDateStr, intervalDaysToEffective } from "@/lib/ciclo-utils";

export default function NovaDispensacao({ paciente, ciclo, onClose, galeriaFotos = [], galeriaLoading = false }: Props) {
  const { user } = useAuth();
  const [showGalleryPicker, setShowGalleryPicker] = useState(false);
  const [pastaNumeroAtual, setPastaNumeroAtual] = useState<number | null>(null);
  useEffect(() => {
    let alive = true;
    fetchPastaNumeroPaciente(paciente.id, paciente.nome)
      .then((info) => { if (alive) setPastaNumeroAtual(info?.numero ?? null); })
      .catch(() => { if (alive) setPastaNumeroAtual(null); });
    return () => { alive = false; };
  ...
  const runCupomOcrExtra = async (base64: string) => {
    ...
    try {
      ...
      if (data.data_compra) {
        let dataFinal: string = data.data_compra;
        let recalculada = false;
        const intervalo = Number(ciclo.intervalo_dias || 30);
        if (data.data_proxima_retirada) {
          try {
            const derivada = addDaysToDateStr(data.data_proxima_retirada, -intervalDaysToEffective(intervalo));
            if (derivada && derivada !== data.data_compra) {
              const validade = getCycleValidityDate(ciclo);
              const futura = isFutureDateStr(derivada);
              const foraValidade = validade && compareDateStr(derivada, validade) > 0;
              if (!futura && !foraValidade) {
                dataFinal = derivada;
                recalculada = true;
              }
            }
        ...
        setCupomValidation({ ...data, data_compra_final: dataFinal, data_compra_recalculada: recalculada });
        setDataDispensacao(dataFinal);
        setDatasConfirmadas(false);
        if (recalculada) {
          toast.info(`Data corrigida via PROX.COM: ${dataFinal.split("-").reverse().join("/")}. Confira antes de registrar.`);
        }
        if (isFutureDateStr(dataFinal)) {
  ...
  const runCupomOCR = async (base64: string) => {
    ...
    try {
      ...
      if (data.data_compra) {
        // Cross-check: se PROX.COM foi lida e o intervalo é conhecido,
        // derivamos a data_compra esperada (PROX.COM - intervaloEfetivo).
        // Cupons térmicos frequentemente têm a borda esquerda cortada, então
        // preferimos o valor derivado quando ele difere do lido.
        let dataFinal: string = data.data_compra;
        let recalculada = false;
        const intervalo = Number(ciclo.intervalo_dias || 30);
        if (data.data_proxima_retirada) {
          try {
            const derivada = addDaysToDateStr(data.data_proxima_retirada, -intervalDaysToEffective(intervalo));
            if (derivada && derivada !== data.data_compra) {
              // Só usa a derivada se ela não for futura e estiver dentro da validade
              const validade = getCycleValidityDate(ciclo);
              const futura = isFutureDateStr(derivada);
              const foraValidade = validade && compareDateStr(derivada, validade) > 0;
              if (!futura && !foraValidade) {
        ...
        setDataDispensacao(dataFinal);
        if (recalculada) {
          toast.info(`Data corrigida via PROX.COM: ${dataFinal.split("-").reverse().join("/")} (a borda do cupom pode ter cortado o 1º dígito). Confira antes de registrar.`);
        }
        if (isFutureDateStr(dataFinal)) {
          toast.warning("A data lida no cupom é futura. Confira se não é a data de próxima retirada e corrija antes de registrar.");
        }
  ...
  const handleRegistrar = async () => {
    ...
    if (!dataDispensacao) { toast.error("Informe a data da dispensação."); return; }
    if (isFutureDateStr(dataDispensacao)) {
      toast.error(`A data da dispensação não pode ser futura. Hoje é ${formatDateBR(getTodayLocalDateStr())}.`);
      return;
    }

    // Validar receita ainda válida (usa validade_ate da receita quando disponível)
    const validadeReceita = getCycleValidityDate(ciclo);
    const hoje = getTodayLocalDateStr();
    if (validadeReceita && compareDateStr(hoje, validadeReceita) > 0) {
      toast.error(`Receita vencida em ${validadeReceita.split("-").reverse().join("/")}. Encerre o ciclo e registre uma nova receita.`);
      return;
    }
    if (validadeReceita && compareDateStr(dataDispensacao, validadeReceita) > 0) {
      toast.error(`A data da dispensação (${formatDateBR(dataDispensacao)}) é posterior à validade da receita (${formatDateBR(validadeReceita)}).`);
      return;
    }

    // Validar limite máximo de retiradas do ciclo
    const totalAtual = Number(ciclo.total_dispensacoes || 0);
    const limiteMax = Number(ciclo.limite_maximo || 0);
    if (limiteMax > 0 && totalAtual >= limiteMax) {
      toast.error(`Limite máximo de ${limiteMax} retiradas atingido neste ciclo. Encerre o ciclo para registrar uma nova receita.`);
      return;
    }
    ...
    try {
      ...
      await supabase.from("ciclos").update(cicloUpdates as any).eq("id", ciclo.id);
      ...
      if (wantsPrint) {
        printDispensacaoCupom({
          ...
          categoria: normalizeCategoria((ciclo as any).receitas?.tipo),
          pastaLetra: normalizeInitial(paciente.nome),
          pastaNumero: pastaNumeroAtual,
  ...
  const canGoNext = () => {
    ...
    if (step === "cupom") return !!cupomFile && !!cupomQrFile && datasConfirmadas && dispensadoHoje !== null && !!dataDispensacao && !isFutureDateStr(dataDispensacao);
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative z-50 w-full max-w-5xl max-h-[92vh] flex flex-col rounded-2xl p-0 border bg-background shadow-lg mx-4">
