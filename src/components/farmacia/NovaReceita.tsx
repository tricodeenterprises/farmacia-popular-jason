...
import { Camera, Upload, Loader2, CheckCircle, AlertCircle, Search, FileText, Shield, Plus, X, ArrowRight, Calendar, User, CreditCard, CheckSquare } from "lucide-react";
...
export default function NovaReceita({ pacienteId, onClose }: Props) {
  ...
  const runOCR = async (base64: string) => {
    ...
    try {
      ...
      if (data.erro && !data.data_emissao && !data.tipo_receita) { toast.error(data.erro); setOcrStatus("error"); setOcrDetails(["❌ " + data.erro, ...getQualityFeedback(data)]); return; }
      ...
      const hasAny = data.data_emissao || data.tipo_receita || data.medicamentos?.length;
  ...
  const isFormComplete = () => !!dataEmissao && !!receitaFile && !getReceitaVencida() && !needsDateConfirmation;
  ...
  return (
    <>
      ...
      <Dialog open onOpenChange={() => onClose()}>
        <DialogContent className="w-full max-w-5xl max-h-[100dvh] sm:max-h-[95vh] overflow-y-auto p-0 rounded-none sm:rounded-2xl border-0 sm:border shadow-xl bg-[hsl(var(--background))] sm:m-4" style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
              ...
              <div className="border rounded-2xl p-4 sm:p-5 bg-background space-y-4 sm:space-y-5">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  ...
                  <div className="rounded-2xl border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
                    <p className="font-semibold text-foreground">Fluxo operacional simples</p>
                    <p className="mt-1 leading-relaxed">O registro exige somente arquivo da receita, tipo e data de emissão. Nenhum dado profissional adicional bloqueia a operação.</p>
                  </div>
                </div>
              </div>
            </div>
